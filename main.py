...
class MongoDBManager:
    ...
    async def _create_indexes(self):
        ...
        try:
            ...
            await self.searches_collection.create_index("created_at", expireAfterSeconds=1800)  # 30 minutes
    ...
    async def get_user_count(self) -> int:
        ...
        if not self._connected:
            return len(user_ids)  # Fallback to in-memory
    ...
    async def get_all_user_ids(self) -> List[int]:
        ...
        if not self._connected:
            return list(user_ids)  # Fallback to in-memory
    ...
    async def check_rate_limit(self, user_id: int, max_requests: int = 5, time_window: int = 60) -> bool:
        ...
        if not self._connected:
            return rate_limiter.is_allowed(user_id)  # Fallback to in-memory
    ...
    async def get_remaining_requests(self, user_id: int, max_requests: int = 5, time_window: int = 60) -> int:
        ...
        if not self._connected:
            return rate_limiter.remaining_requests(user_id)  # Fallback to in-memory
    ...
    async def cleanup_expired_data(self):
        ...
        if not self._connected:
            cleanup_cache()  # Fallback to in-memory cleanup
            return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_ids.add(user_id)  # Also add to in-memory set as fallback
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check rate limit using MongoDB or fallback
    ...
    # Get user IDs from MongoDB or fallback
...
async def search_books(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not results:
            # Fallback to in-memory cache if MongoDB cache is empty
...
async def main():
    ...
    try:
        ...
        logger.info(f"MongoDB status: {'Connected' if db_manager._connected else 'Disconnected'}")
